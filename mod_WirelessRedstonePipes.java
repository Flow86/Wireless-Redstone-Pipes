...
public class mod_WirelessRedstonePipes extends BaseModMp {
	...
	public void ModsLoaded () {
		...
		this.config = new Configuration(new File(CoreProxy.getBuildCraftBase(), "config/wirelessredstonepipes.cfg"), false);
	
		Configuration.Property wirelessBouncePipeBlockId = this.config.getOrCreateBlockIdProperty("wirelessBouncePipe", 160);
		
		this.config.save();
		
		if (Integer.parseInt(wirelessBouncePipeBlockId.value) != 0)
		{
			wirelessBouncePipeBlock = new BlockWirelessBouncePipe(Integer.parseInt(wirelessBouncePipeBlockId.value));
			CoreProxy.addName(wirelessBouncePipeBlock.setBlockName("wirelessBouncePipe"), "Wireless Bounce Pipe");
			ModLoader.RegisterBlock(wirelessBouncePipeBlock);
			ModLoader.AddRecipe(new ItemStack(wirelessBouncePipeBlock, 8), new Object[] { 
				"CSC", 
				Character.valueOf('C'), Block.cobblestone, 
				Character.valueOf('S'), mod_WirelessRedstone.blockWirelessR 
			});
		
			ModLoader.RegisterTileEntity(TileWirelessBouncePipe.class, "WirelessBouncePipe");
		}
